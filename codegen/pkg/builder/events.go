...
func (b *Builder) collectEvents() error {
	...
	for eventType, pathItem := range b.spec.Webhooks.FromOldest() {
		...
		objectKind, ok := extension.Get[string](operation.Extensions, "x-object-type")
		if !ok || strings.TrimSpace(objectKind) == "" {
			return fmt.Errorf("event %q is missing x-object-type", eventType)
		}
		...
		events = append(events, EventDefinition{
			RegistrationMethod: "on_" + strcase.ToSnake(name),
