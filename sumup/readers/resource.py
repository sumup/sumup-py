...
from ..types import (
    ...
    ReaderPaymentRequestParamsInput,
...
CreateReaderCheckoutBodyCardTypeInput = typing.Literal["credit", "debit"] | str
...
class ReadersResource(Resource):
    ...
    def create_go_checkout(
        ...
        *,
        ...
        Use `client_transaction_id` as an idempotency key: retrying the request with the same value returns the result ofthe original payment instead of creating a duplicate.
        ...
        body_data["client_transaction_id"] = client_transaction_id
        if not isinstance(tip_amount, NotGivenType):
            body_data["tip_amount"] = tip_amount
        body_data["total_amount"] = total_amount

        resp = self._client.post(
            f"/v0/merchants/{merchant_code}/readers/{reader_id}/go-checkout",
            json=serialize_request_data(body_data),
            headers=headers,
        )
        if resp.status_code == 200:
            return pydantic.TypeAdapter(ReaderPaymentResponse).validate_python(resp.json())
    ...
    def get(
        self, merchant_code: str, reader_id: ReaderId, headers: HeaderTypes | None = None
    ) -> Reader:
        """
        Retrieve a Reader
    ...
    def delete(self, merchant_code: str, reader_id: ReaderId, headers: HeaderTypes | None = None):
        """
        Delete a reader
    ...
    def update(
        ...
        reader_id: ReaderId,
        *,
        name: ReaderNameInput | None | NotGivenType = NOT_GIVEN,
        metadata: MetadataInput | None | NotGivenType = NOT_GIVEN,
        headers: HeaderTypes | None = None,
    ) -> Reader:
        """
    ...
    def create_checkout(
        ...
        reader_id: str,
        *,
        aade: CreateReaderCheckoutBodyAadeInput | None | NotGivenType = NOT_GIVEN,
        affiliate: CreateReaderCheckoutBodyAffiliateInput | None | NotGivenType = NOT_GIVEN,
        card_type: CreateReaderCheckoutBodyCardTypeInput | None | NotGivenType = NOT_GIVEN,
        ...
        tip_rates: typing.Sequence[float] | None | NotGivenType = NOT_GIVEN,
        tip_timeout: int | None | NotGivenType = NOT_GIVEN,
    ...
    def get_status(
        self, merchant_code: str, reader_id: str, headers: HeaderTypes | None = None
    ) -> StatusResponse:
        """
        Get a Reader Status
    ...
    def get_checkout(
        ...
        """
        resp = self._client.get(
            f"/v0.1/merchants/{merchant_code}/readers/{reader_id}/checkout/{checkout_id}",
            headers=headers,
        )
        if resp.status_code == 200:
            return pydantic.TypeAdapter(GetReaderCheckoutResponse).validate_python(resp.json())
        elif resp.status_code == 401:
            raise APIError("Unauthorized", status=resp.status_code, body=resp.text)
        elif resp.status_code == 404:
            raise APIError(
                "Response when given reader or checkout is not found",
                status=resp.status_code,
                body=resp.text,
            )
        else:
